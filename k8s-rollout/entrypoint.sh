...
ssh $INPUT_DEPLOY_USER@$INPUT_DEPLOY_HOST -p $INPUT_DEPLOY_PORT -i $JET_DEPLOY_KEY_PATH -o StrictHostKeyChecking=no -T "kubectl set image ${INPUT_RESOURCE} ${INPUT_CONTAINER}=${INPUT_REGISTRY}/${INPUT_IMAGE}:${INPUT_TAG}"
